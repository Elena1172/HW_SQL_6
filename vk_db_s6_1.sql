...
дата создания записи, название таблицы, идентификатор первичного ключа. (Триггеры)*/
...
DROP PROCEDURE IF EXISTS add_mess;
DELIMITER $$
CREATE PROCEDURE add_communities
(name_ VARCHAR(30), OUT res VARCHAR(400))
BEGIN
	DECLARE rollback_ BIT DEFAULT 0;
    DECLARE code_error VARCHAR(300);
    DECLARE text_error VARCHAR(400);
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION
    BEGIN 
		SET rollback_ = 1;
        GET STACKED DIAGNOSTICS CONDITION 1
			code_error = RETURNED_SQLSTATE, text_error = MESSAGE_TEXT;
	END;
    START TRANSACTION;
    INSERT INTO communities (name )
    VALUES (name_);
    IF rollback_ THEN SET res = CONCAT('Ошибка! Код ошибки:', code_error, ' Текст ошибки:', text_error);
		ROLLBACK;
	ELSE SET res = 'Ok';
		COMMIT;
	END IF;
...
( firstname VARCHAR(50), lastname VARCHAR(50), email VARCHAR(120), OUT res VARCHAR(400))
BEGIN
	DECLARE rollback_ BIT DEFAULT 0;
    DECLARE code_error VARCHAR(300);
    DECLARE text_error VARCHAR(400);
...
END;
CREATE PROCEDURE add_mess
( from_user_id INT, to_user_id INT, body text,  OUT res VARCHAR(400))
BEGIN
	DECLARE rollback_ BIT DEFAULT 0;
    DECLARE code_error VARCHAR(300);
    DECLARE text_error VARCHAR(400);
    ...
    VALUES (from_user_id , to_user_id, body, now());
    IF rollback_ THEN SET res = CONCAT('Ошибка! Код ошибки:', code_error, ' Текст ошибки:', text_error);
		ROLLBACK;
	ELSE SET res = 'Ok';
		COMMIT;
...
CREATE TRIGGER tr_insert_user AFTER INSERT ON users
	FOR EACH ROW BEGIN
		...
		SET tmp_id=new.id;
		INSERT INTO logs(created_at, table_name, log_id) values (now(), 'users', tmp_id);
	end $$

DROP TRIGGER IF EXISTS tr_insert_com $$
CREATE TRIGGER tr_insert_com AFTER INSERT ON communities
	FOR EACH ROW BEGIN
		DECLARE tmp_id BIGINT(20) UNSIGNED;
		SET tmp_id=new.id;
		INSERT INTO logs(created_at, table_name, log_id) values (now(), 'communities', tmp_id);
	end $$
    
    DROP TRIGGER IF EXISTS tr_insert_mess $$
CREATE TRIGGER tr_insert_mess AFTER INSERT ON messages
	FOR EACH ROW BEGIN
		DECLARE tmp_id BIGINT(20) UNSIGNED;
		SET tmp_id=new.id;
		INSERT INTO logs(created_at, table_name, log_id) values (now(), 'messages', tmp_id);
	end $$
 
DELIMITER ;
CALL add_communities('sport', @r);
SELECT @r;
CALL add_communities('running', @r);
SELECT @r;
CALL add_communities('sport', @r);
SELECT @r;
CALL add_user('Alex', 'Bond', 'ww@w', @result);
SELECT @result;
CALL add_user('Andry', 'Black', 'ww@w1', @result);
SELECT @result;
CALL add_user('Max', 'Black', 'ww@w1', @result);
SELECT @result;
CALL add_mess(1, 2, 'text_text_', @r);
SELECT @r;
CALL add_mess(3, 9, 'text_text_text', @r);
SELECT @r;