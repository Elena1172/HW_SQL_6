DROP DATABASE IF EXISTS vk_db_s6;
CREATE DATABASE vk_db_s6;
USE vk_db_s6;
...
DROP PROCEDURE IF EXISTS friendship_of;
DROP FUNCTION IF EXISTS friendship_dir;
DROP FUNCTION IF EXISTS rank_user;
DELIMITER $$
CREATE PROCEDURE friendship_of(for_user_id BIGINT)
BEGIN
WITH friends AS (
...
CREATE FUNCTION friendship_dir(id BIGINT)
RETURNS FLOAT READS SQL DATA 
BEGIN
 DECLARE to_user INT; -- заявок к пользователю
 DECLARE from_user INT; -- заявок от пользователя
 DECLARE res FLOAT;
 SELECT count(*) INTO to_user FROM friend_requests
 WHERE target_user_id = id;
 SELECT count(*) INTO  from_user
 FROM friend_requests WHERE initiator_user_id = id; 
 IF from_user <> 0 THEN
 SET res =  to_user / from_user;
 ELSE SET res = 0;
 END IF;
 RETURN res;
END$$

-- еще написала функцию которая возвращает ранг по количеству одобренных запросов
-- не сразу поняла  суть задания
CREATE FUNCTION rank_user(n INT)
RETURNS INT
DETERMINISTIC
BEGIN
DECLARE a INT; 
SELECT tt.rank_ INTO a FROM
(SELECT count_, DENSE_RANK() OVER(ORDER BY count_ DESC)AS rank_ FROM
(SELECT initiator_user_id AS id, COUNT(*) AS count_
FROM friend_requests WHERE status = 'approved'
GROUP BY initiator_user_id) AS t) AS tt
WHERE count_ = n
LIMIT 1;
return a;
END $$
...
DELIMITER ;
CALL friendship_of(3);
SELECT friendship_dir(3);
SELECT rank_user(2);
SELECT rank_user(1);
SELECT rank_user(3);